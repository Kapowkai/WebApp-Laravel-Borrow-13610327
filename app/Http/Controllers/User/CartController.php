...
class CartController extends Controller
{
    ...
    public function addToCart($id) {
        ...
        if($item_exits > 0){
            return response()->json(['message'=>'มี item อยู่แล้ว','status'=>302]);
        }
        $data = [
        ...
        return response()->json(['message'=>'เพ่ิมลงไปในตะกร้าเรียบร้อยแล้ว','status'=>200]);
    }

    public function removeInCart($id) {
        Cart::Where('user_id',auth()->user()->id)->Where('item_id',$id)->delete();

        return response()->json(['message'=>'ลบในตะกร้าเรียบร้อยแล้ว','status'=>200]);
    }
    public function clearCart() {
        Cart::Where('user_id',auth()->user()->id)->delete();

        return response()->json(['message'=>'clear ในตะกร้าเรียบร้อยแล้ว','status'=>200]);
    }
